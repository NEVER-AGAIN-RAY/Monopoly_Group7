...
public final class AiHeuristics {
        ...
        for (Card c : bot.getHandCardsView()) {
            ...
            if (c instanceof PropertyWildCard wild) {
                for (String color : wildDeployColors(bot, wild, profile)) {
                    ...
                    out.add(new AiPlayCandidate("c" + seq++, deploy, describeWildDeploy(bot, color)));
                }
                continue;
            }
    ...
    private static int deployCompletionScore(AIPlayer bot, String colorKey) {
        String key = normalizeColor(colorKey);
        if (key.isBlank()) {
            return 0;
        }
        int need = propertyNeed(key);
        int eff = propertyCount(bot, key);
        int after = eff + 1;
        if (eff >= need) {
            return -1000 - eff;
        }
    ...
    private static int propertyCount(AIPlayer bot, String colorKey) {
        if (bot == null) {
            return 0;
        }
        String key = normalizeColor(colorKey);
        ...
        return PropertySetCalculator.effectiveCountForColor(bot.getPropertyCardsView(), key);
    }

    private static int propertyNeed(String colorKey) {
        String key = normalizeColor(colorKey);
        return PropertySetCalculator.REQUIRED_BY_COLOR.getOrDefault(key, 3);
    }

    private static String describeWildDeploy(AIPlayer bot, String colorKey) {
        String key = normalizeColor(colorKey);
        if (!includeWildProgressInSummary()) {
            return "Deploy wild property as " + key + ".";
        }
        return "Deploy wild property as " + key
                + " wildProgressScore=" + deployCompletionScore(bot, key)
                + " current=" + propertyCount(bot, key)
                + " need=" + propertyNeed(key) + ".";
    }

    private static boolean includeWildProgressInSummary() {
        return Boolean.parseBoolean(System.getProperty(
                "monopoly.ai.includeWildProgressInSummary", "false"));
    }
    ...
    private static String pickWildColor(AIPlayer bot, AiStrategyProfile profile, Random rng) {
        ...
        for (String color : keys) {
            int score = deployCompletionScore(bot, color);
            if (score > bestScore) {
                bestScore = score;
                best = color;
            }
    ...
    private static String pickWildDeployColor(
        ...
        for (String color : printed) {
            ...
            int score = deployCompletionScore(bot, key);
            if (score > bestScore) {
                bestScore = score;
                best = key;
            }
    ...
    private static String describeCandidate(
        ...
        if ("STEAL_PROPERTY".equals(effect)) {
            StealOption option = stealOptionFor(bot, context, req);
            if (option != null) {
                return "Action STEAL_PROPERTY target=" + req.getTargetPlayerId()
                        + " card=" + option.take().getId()
                        + " takeColor=" + option.takeColor()
    ...
    private static StealOption stealOptionFor(
            AIPlayer bot,
            GameContext context,
            PlayActionRequest req) {
        if (req == null) {
        ...
        Player target = context == null ? null : context.findPlayer(req.getTargetPlayerId());
        PropertyCard take = findPropertyById(target, req.getTargetCardId());
        if (target == null || take == null) {
            return null;
        }
        String takeColor = propertyColorForSet(take);
        int takeValue = Math.max(1, PayableCards.valueOf(take));
        int completionGain = completionDeltaAfterTrade(bot, takeColor, null);
        int oppCompletionLoss = -completionDeltaAfterTrade(target, null, takeColor);
        return new StealOption(
                target.getPlayerId(),
                take,
                takeColor,
                takeValue,
    ...
    private static List<String> wildDeployColors(AIPlayer bot, PropertyWildCard wild, AiStrategyProfile profile) {
        ...
        colors.sort(Comparator
                .comparingInt((String color) -> deployCompletionScore(bot, color))
                .reversed());
        return colors;
    }
