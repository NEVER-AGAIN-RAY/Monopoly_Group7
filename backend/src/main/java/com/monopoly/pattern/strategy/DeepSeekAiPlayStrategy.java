...
public class DeepSeekAiPlayStrategy implements AiPlayStrategy, AiChoiceAdvisor {
    ...
    private static final boolean INCLUDE_SEARCH_PRIOR =
            Boolean.parseBoolean(System.getProperty("monopoly.deepseek.includeSearchPrior", "true"));
    ...
    private static final String STRATEGY_CONTEXT = """
            ...
            - Candidate tactics include localScore from a hard-tested local evaluator. Treat localScore as a
              strong prior: normally choose one of the highest localScore candidates. Choose a candidate more
              than 500 points lower only when it immediately wins, blocks an immediate opponent win, or creates
              a clearly superior complete-set path that the localScore fields miss.
            - When decision.searchPrior.available is true, searchScore/searchRank come from the current local
              champion one-ply lookahead evaluator. Prefer searchRank 1 unless there is an immediate win/block
              reason. Do not override a much higher searchScore for vague long-term intuition.
            """;
    ...
    public boolean tryPlayOneCard(AIPlayer bot, GameContext context, AiGameBridge bridge) {
        ...
        JsonObject searchPrior = searchPrior(bridge, bot, candidates);
        ChoiceResult choice = chooseCandidate(bot, context, candidates, searchPrior);
        String candidateId = choice.choiceId();
        AiHeuristics.AiPlayCandidate chosen = findCandidate(candidates, candidateId);
        boolean localFallbackUsed = false;
        if (chosen == null) {
        ...
        try {
            ...
            recordPlayTrace(bot, context, candidates, chosen, choice, localFallbackUsed, searchPrior);
    ...
    private ChoiceResult chooseCandidate(
            AIPlayer bot,
            GameContext context,
            List<AiHeuristics.AiPlayCandidate> candidates,
            JsonObject searchPrior) {
        String prompt = buildUserPrompt(bot, context, candidates, searchPrior);
        try {
            ChoiceResult first = requestCandidateId(prompt, candidates);
            String id = first.choiceId();
            ...
            metadata.addProperty("errorClass", ex.getClass().getSimpleName());
            metadata.addProperty("errorMessage", ex.getMessage());
            return new ChoiceResult(null, null, metadata);
        }
    }

    private ChoiceResult requestCandidateId(
            String prompt,
            List<AiHeuristics.AiPlayCandidate> candidates)
        ...
        try {
            ...
            return choiceFromRaw(raw, candidates, preferredAttempted, "preferred");
            ...
            ChoiceResult result = choiceFromRaw(raw, candidates, false, "fallback_after_transport");
            JsonObject metadata = result.metadata();
            metadata.addProperty("retryReasonClass", transport.getClass().getSimpleName());
            metadata.addProperty("retryReasonMessage", transport.getMessage());
            return new ChoiceResult(result.choiceId(), result.rawResponse(), metadata);
            ...
            String retryPrompt = prompt + "\n\nReturn valid JSON only. No markdown. "
                    + "The candidateId must be one of: " + candidateIds(candidates) + ".";
            String raw = client.completeFallback(SYSTEM_PROMPT, retryPrompt, true);
            ChoiceResult result = choiceFromRaw(raw, candidates, false, "fallback_after_malformed");
            JsonObject metadata = result.metadata();
            metadata.addProperty("retryReasonClass", malformed.getClass().getSimpleName());
            metadata.addProperty("retryReasonMessage", malformed.getMessage());
            return new ChoiceResult(result.choiceId(), result.rawResponse(), metadata);
        }
    }
    ...
    private ChoiceResult choiceFromRaw(
            String raw,
            List<AiHeuristics.AiPlayCandidate> candidates,
            boolean preferredAttempted,
            String source) {
        JsonObject metadata = new JsonObject();
        metadata.addProperty("source", "deepseek");
        metadata.addProperty("requestSource", source);
        JsonObject usage = client.consumeLastUsage();
        if (usage.size() > 0) {
            metadata.add("usage", usage);
        }
        try {
            JsonObject json = parseDecisionJson(raw);
            if (preferredAttempted) {
            ...
            return new ChoiceResult(string(json, "candidateId"), raw, metadata);
        } catch (RuntimeException malformed) {
            if (preferredAttempted) {
                client.recordPreferredJsonFailure(malformed.getClass().getSimpleName()
                        + " " + malformed.getMessage());
            ...
            if (salvaged != null) {
                ...
                metadata.addProperty("salvagedFromMalformed", true);
                metadata.addProperty("malformedClass", malformed.getClass().getSimpleName());
                metadata.addProperty("malformedMessage", malformed.getMessage());
                return new ChoiceResult(salvaged, raw, metadata);
            }
            throw malformed;
        }
    }
    ...
    private void recordPlayTrace(
            ...
            JsonObject searchPrior) {
        if (bot == null || chosen == null || traceSink == null || traceSink == DecisionTraceSink.NONE) {
            return;
        }
        String actorId = bot.getPlayerId();
        long seq = TRACE_SEQUENCE.incrementAndGet();
        JsonObject contextJson = JsonParser.parseString(
                buildUserPrompt(bot, context, candidates, searchPrior)).getAsJsonObject();
        SimulationDecisionRequest request = new SimulationDecisionRequest(
                traceSessionId + "-" + actorId + "-DEEPSEEK-" + seq,
                traceSessionId,
                actorId,
                "PLAY_CARD",
        ...
        JsonObject metadata = choice == null ? new JsonObject() : choice.metadata();
        metadata.addProperty("source", "deepseek");
        metadata.addProperty("localFallbackUsed", localFallbackUsed);
        AiHeuristics.AiPlayCandidate localBest = bestLocalCandidate(candidates);
        if (localBest != null) {
            metadata.addProperty("localBestChoiceId", localBest.id());
            metadata.addProperty("localBestScore", candidateScore(localBest));
        }
        JsonObject candidateScores = new JsonObject();
        for (AiHeuristics.AiPlayCandidate candidate : candidates) {
            candidateScores.addProperty(candidate.id(), candidateScore(candidate));
        }
        metadata.add("localCandidateScores", candidateScores);
        if (searchPrior != null && searchPrior.size() > 0) {
            metadata.add("searchPrior", searchPrior.deepCopy());
        }
        try {
            traceSink.record(request, new SimulationDecisionResult(
                    request.getDecisionId(),
                    chosen.id(),
                    choice == null ? null : choice.rawResponse(),
                    metadata));
        } catch (RuntimeException ex) {
            AiBattleLogger.log("DeepSeek",
    ...
    private static List<SimulationDecisionCandidate> traceCandidates(
        ...
        for (AiHeuristics.AiPlayCandidate candidate : candidates) {
            out.add(new SimulationDecisionCandidate(
                    candidate.id(),
                    candidate.summary(),
                    requestJson(candidate.request())));
        }
        return out;
    }

    private static JsonObject searchPrior(
            AiGameBridge bridge,
            AIPlayer bot,
            List<AiHeuristics.AiPlayCandidate> candidates) {
        if (!INCLUDE_SEARCH_PRIOR || !(bridge instanceof GameController controller)) {
            return new JsonObject();
        }
        try {
            return SearchLookaheadAiPlayStrategy.promptPrior(controller, bot, candidates);
        } catch (RuntimeException ex) {
            ...
            prior.addProperty("error", ex.getClass().getSimpleName() + ": " + ex.getMessage());
            return prior;
        }
    }

    private static JsonObject requestJson(PlayActionRequest request) {
        if (request == null) {
            return new JsonObject();
        }
    ...
    static String buildUserPrompt(
            ...
            GameContext context,
            List<AiHeuristics.AiPlayCandidate> candidates) {
        return buildUserPrompt(bot, context, candidates, new JsonObject());
    }
    ...
    static String buildUserPrompt(
        ...
        decision.add("searchPrior", searchPrior == null ? new JsonObject() : searchPrior.deepCopy());
        ...
        for (AiHeuristics.AiPlayCandidate c : candidates) {
            ...
            addSearchPriorToCandidate(row, searchPrior, c.id());
            cands.add(row);
        }
    ...
    private static void addSearchPriorToCandidate(
            JsonObject candidateRow,
            JsonObject searchPrior,
            String candidateId) {
        if (candidateRow == null || searchPrior == null || candidateId == null
                || !searchPrior.has("candidateScores")) {
            return;
        }
        JsonObject scores = searchPrior.getAsJsonObject("candidateScores");
        if (scores != null && scores.has(candidateId)) {
            candidateRow.addProperty("searchScore", scores.get(candidateId).getAsDouble());
        }
        JsonObject ranks = searchPrior.has("candidateRanks")
                ? searchPrior.getAsJsonObject("candidateRanks")
                : null;
        if (ranks != null && ranks.has(candidateId)) {
            candidateRow.addProperty("searchRank", ranks.get(candidateId).getAsInt());
        }
    }
    ...
    private static JsonArray candidateSelectionProtocolJson(AIPlayer bot, GameContext context) {
        ...
        arr.add("Use tactics.localScore as the tie-break anchor. If a candidate is 500+ localScore below the best legal candidate, reject it unless it wins now, blocks a win, or clearly completes/protects your set race.");
        arr.add("If decision.searchPrior.available is true, treat candidate.searchRank/searchScore as the strongest numeric prior; searchRank 1 is the champion lookahead recommendation.");
        if (teamAwareMode()) {
