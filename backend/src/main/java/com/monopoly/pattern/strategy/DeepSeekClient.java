...
public final class DeepSeekClient {
    ...
    public static String configuredApiKeyForTest() throws IOException {
        return apiKey();
    }
    ...
    private static String openAiServiceTier() {
        ...
        raw = raw == null ? "" : raw.trim().toLowerCase(java.util.Locale.ROOT);
        if ("fast".equals(raw)) {
            return isOfficialOpenAiBaseUrl() ? "auto" : "";
        }
        return raw;
    }

    private static boolean isOfficialOpenAiBaseUrl() {
        String url = baseUrl().toLowerCase(java.util.Locale.ROOT);
        return url.equals(DEFAULT_OPENAI_BASE_URL) || url.startsWith(DEFAULT_OPENAI_BASE_URL + "/");
    }
