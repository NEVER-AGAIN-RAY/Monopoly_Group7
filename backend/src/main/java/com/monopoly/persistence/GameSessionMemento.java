...
public final class GameSessionMemento {
    ...
    private static AiPlayStrategy resolveAiStrategy(String diff) {
        ...
        return switch (d) {
            ...
            case "LOOKAHEAD", "SEARCH", "LOCAL_STRONG", "STRONG" -> new SearchLookaheadAiPlayStrategy();
    ...
    private static String normalizeGameMode(String mode) {
        String m = mode == null ? "" : mode.trim().toUpperCase(java.util.Locale.ROOT);
        return m.isBlank() ? "HVM" : m;
    }
