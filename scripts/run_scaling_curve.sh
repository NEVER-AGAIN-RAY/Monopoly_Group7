...
TRACE="${1:?Usage: scripts/run_scaling_curve.sh data/distillation/run.jsonl models/distillation/scaling-run}"
OUT_PREFIX="${2:?Usage: scripts/run_scaling_curve.sh data/distillation/run.jsonl models/distillation/scaling-run}"
...
python3 scripts/make_scaling_subsets.py "$TRACE" \
...
for subset in manifest["subsets"]:
    ...
    cmd = ["scripts/train_distilled_rankers.sh", subset_path, model_prefix]
