...
MODEL="${1:?Usage: training/scripts/evaluate_gameplay_matrix.sh path/to/candidate_ranker_mlp.json [output-dir]}"
OUT_DIR="${2:-$ROOT/backend/models/distillation/gameplay-matrix-$(date +%Y%m%d-%H%M%S)}"
...
for players in "${PLAYERS_ARRAY[@]}"; do
  ...
  for opponent in "${OPPONENTS_ARRAY[@]}"; do
    ...
    for seat in "${SEATS_ARRAY[@]}"; do
      ...
      training/scripts/evaluate_distilled_ranker.sh "$MODEL" "$GAMES" "$players" "$SNAPSHOTS" > "$out"
    done
  done
done

python3 training/scripts/summarize_gameplay_matrix.py "$OUT_DIR" \
