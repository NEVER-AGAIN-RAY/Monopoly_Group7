...
TRACE="${1:?Usage: training/scripts/package_training_handoff.sh training/data/distillation/run.jsonl backend/models/distillation/run-prefix [archive-path]}"
OUT_PREFIX="${2:?Usage: training/scripts/package_training_handoff.sh training/data/distillation/run.jsonl backend/models/distillation/run-prefix [archive-path]}"
...
for path in \
  ...
  "backend/models/distillation/local-enhanced-20260524-strategic-probe-selection_report.json" \
  "backend/models/distillation/local-enhanced-20260524-strategic-probe-relabel_summary.json" \
  ...
  "backend/models/distillation/local-enhanced-20260524-strategic-probe-linear/metrics.json" \
  "backend/models/distillation/local-enhanced-20260524-strategic-probe-linear/candidate_ranker_linear.json" \
  "backend/models/distillation/local-enhanced-20260524-strategic-probe-knn/metrics.json" \
...
mkdir -p "$TMP_DIR/monopoly-training-handoff/training/scripts" "$TMP_DIR/monopoly-training-handoff/docs"
cp training/scripts/distill_dataset.py \
   ...
   training/scripts/summarize_gameplay_matrix.py \
   training/scripts/make_scaling_subsets.py \
   ...
   "$TMP_DIR/monopoly-training-handoff/training/scripts/"
...
training/scripts/train_distilled_rankers.sh "$TRACE" "$OUT_PREFIX"
\`\`\`
...
For multi-seed reporting, run \`MONOPOLY_SEEDS=11,42,73 training/scripts/run_seed_replicates.sh "$TRACE" "$AGG_PREFIX"\` and inspect \`$AGG_PREFIX-seed_summary.md\`. If the archive already contains \`*-seed_summary.md\` or \`*-gameplay_matrix.md\`, read those first for the aggregate result.
...
- Run \`python3 training/scripts/check_ai_quality_gate.py --require production --production-prefix <prefix>\` before calling a dataset production-ready.
EOF
