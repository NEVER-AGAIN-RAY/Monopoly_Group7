...
INPUT="${1:?Usage: training/scripts/relabel_distillation_trace.sh input.jsonl output.jsonl [output-prefix]}"
OUTPUT="${2:?Usage: training/scripts/relabel_distillation_trace.sh input.jsonl output.jsonl [output-prefix]}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT_PREFIX="${3:-$ROOT/backend/models/distillation/relabel-$STAMP}"
...
if [[ "$SELECT" == "true" || "$SELECT" == "1" || "$SELECT" == "yes" ]]; then
  python3 training/scripts/select_relabel_trace.py "$INPUT" \
...
mvn -q compile exec:java \
  ...
  -Dmonopoly.deepseek.timeoutSeconds="${MONOPOLY_DEEPSEEK_TIMEOUT_SECONDS:-18}" \
  -Dmonopoly.deepseek.maxAttempts="${MONOPOLY_DEEPSEEK_MAX_ATTEMPTS:-3}" \
  -Dmonopoly.deepseek.batch.maxRecentEvents="${MONOPOLY_DEEPSEEK_BATCH_MAX_RECENT_EVENTS:-6}" \
  -Dmonopoly.deepseek.batch.maxStrategicWarnings="${MONOPOLY_DEEPSEEK_BATCH_MAX_STRATEGIC_WARNINGS:-6}" \
  | tee "$RELABEL_LOG"
...
python3 training/scripts/dataset_manifest.py "$OUTPUT" \
  --output "$OUT_PREFIX-dataset_manifest.json" >/dev/null

if [[ "$TEACHER" == "deepseek" ]]; then
  python3 training/scripts/audit_distillation_trace.py "$OUTPUT" \
  ...
  python3 training/scripts/report_distillation_quality.py "$OUTPUT" \
    --source deepseek \
    --min-rows "${MONOPOLY_MIN_DEEPSEEK_ROWS:-1}" \
    --output "$OUT_PREFIX-quality_report.md" \
    --json-output "$OUT_PREFIX-quality_report.json"
else
  python3 training/scripts/distill_dataset.py "$OUTPUT" \
    --mode validate \
