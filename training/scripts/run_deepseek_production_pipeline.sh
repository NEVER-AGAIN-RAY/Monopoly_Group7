...
TRACE_DIR="${MONOPOLY_PRODUCTION_TRACE_DIR:-$ROOT/training/data/distillation}"
MODEL_DIR="${MONOPOLY_PRODUCTION_MODEL_DIR:-$ROOT/backend/models/distillation}"
...
if [[ -f "$PROBE_TRACE" && -f "$PROBE_PREFIX-trace_audit.json" && "${MONOPOLY_RECOLLECT_PROBE:-false}" != "true" ]]; then
  ...
  training/scripts/run_paid_probe.sh "$PROBE_TRACE" "$PROBE_PREFIX"
fi
...
if [[ -f "$MAIN_TRACE" && "${MONOPOLY_RECOLLECT_MAIN:-false}" != "true" ]]; then
  ...
  training/scripts/collect_deepseek_distillation.sh "$MAIN_TRACE"
fi
...
python3 training/scripts/audit_distillation_trace.py "$MERGED_TRACE" \
...
training/scripts/train_distilled_rankers.sh "$MERGED_TRACE" "$OUT_PREFIX"

training/scripts/package_distillation_artifacts.sh "$OUT_PREFIX" "$ARTIFACT_ARCHIVE"
training/scripts/package_training_handoff.sh "$MERGED_TRACE" "$OUT_PREFIX" "$HANDOFF_ARCHIVE"
