...
TRACE="${1:-$ROOT/training/data/distillation/smoke.jsonl}"
OUT_DIR="${2:-$ROOT/backend/models/distillation/smoke}"
...
python3 training/scripts/distill_dataset.py "$TRACE" --output-dir "$OUT_DIR" --epochs 8
python3 training/scripts/distill_dataset.py "$TRACE" --output-dir "$OUT_DIR-linear" --epochs 8 --model-type linear
training/scripts/evaluate_local_ranker.sh "$OUT_DIR-linear/candidate_ranker_linear.json" 2 40
