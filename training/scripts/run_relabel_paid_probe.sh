...
INPUT="${1:-$ROOT/training/data/distillation/local-baseline-20260524-080604.jsonl}"
TRACE="${2:-$ROOT/training/data/distillation/deepseek-relabel-probe-$STAMP.jsonl}"
OUT_PREFIX="${3:-$ROOT/backend/models/distillation/deepseek-relabel-probe-$STAMP}"
...
training/scripts/relabel_distillation_trace.sh "$INPUT" "$TRACE" "$OUT_PREFIX"
