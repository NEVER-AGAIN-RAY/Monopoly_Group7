...
DEFAULT_LOCAL_PREFIX = Path("backend/models/distillation/local-enhanced-20260524-multiseed")
DEFAULT_TRACE = Path("training/data/distillation/local-enhanced-20260524.jsonl")
...
def build_report(
    ...
    return {
        ...
        "validationCommands": [
            ...
            "python3 training/scripts/check_ai_delivery_status.py --output backend/models/distillation/local-enhanced-20260524-multiseed-delivery_status.json",
...
def matrix_block(matrix: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "path": "backend/models/distillation/local-enhanced-20260524-multiseed-gameplay-matrix-smoke/summary.json"
        if matrix else "",
...
def quality_gate_block(quality_gate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "path": "backend/models/distillation/local-enhanced-20260524-multiseed-quality_gate.json"
...
def next_commands(trace: Path, production_prefix: Path | None) -> List[Dict[str, str]]:
    production_name = str(production_prefix or Path("backend/models/distillation/deepseek-run1"))
    return [
        {
            "label": "Refresh local status report",
            "command": "python3 training/scripts/check_ai_delivery_status.py --output backend/models/distillation/local-enhanced-20260524-multiseed-delivery_status.json",
        },
        {
            "label": "Fast paid relabel probe when DeepSeek key is available",
            "command": (
                "DEEPSEEK_API_KEY=... training/scripts/run_relabel_paid_probe.sh "
                f"{trace} training/data/distillation/deepseek-relabel-probe.jsonl backend/models/distillation/deepseek-relabel-probe"
            ),
        },
        {
            ...
            "command": (
                ...
                "MONOPOLY_PAID_CONFIRM=run-paid-overnight training/scripts/run_deepseek_production_pipeline.sh"
            ),
        },
        {
            ...
            "command": (
                ...
                f"training/scripts/run_seed_replicates.sh training/data/distillation/deepseek-run1-merged.jsonl {production_name}"
            ),
        },
        {
            ...
            "command": (
                ...
                f"training/scripts/evaluate_gameplay_matrix.sh {production_name}-seed73-mlp/candidate_ranker_mlp.json "
