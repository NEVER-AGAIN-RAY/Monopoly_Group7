...
TRACE="${1:?Usage: training/scripts/train_distilled_rankers.sh training/data/distillation/deepseek-run.jsonl [output-prefix]}"
OUT_PREFIX="${2:-$ROOT/backend/models/distillation/$(basename "$TRACE" .jsonl)}"
...
python3 training/scripts/distill_dataset.py "$TRACE" \
  --mode validate \
...
python3 training/scripts/audit_distillation_trace.py "$TRACE" "${AUDIT_ARGS[@]}"
...
python3 training/scripts/check_training_readiness.py "$TRACE" "$OUT_PREFIX" \
  --mode "$READINESS_MODE" \
  --min-rows "$MIN_ROWS" \
  --min-rare-kind-rows "$READINESS_MIN_RARE" \
...
python3 training/scripts/summarize_training_run.py "$OUT_PREFIX" \
  --gameplay "$GAMEPLAY_REPORT" \
